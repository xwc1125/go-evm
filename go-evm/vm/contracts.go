...
import (
	"crypto/sha256"
	"errors"
	"github.com/xwc1125/go-evm/go-evm/vm/common/math"
	...
	"github.com/xwc1125/go-evm/pkg/crypto/bn256"
	"math/big"
...
func (c *ecrecover) Run(input []byte) ([]byte, error) {
	...
	input = ioutil.RightPadBytes(input, ecRecoverInputLength)
	...
	return ioutil.LeftPadBytes(keccak.Keccak256(pubKey[1:])[12:], 32), nil
}
...
func (c *ripemd160hash) Run(input []byte) ([]byte, error) {
	...
	ripemd.Write(input)
	return ioutil.LeftPadBytes(ripemd.Sum(nil), 32), nil
}
...
func (c *bigModExp) Run(input []byte) ([]byte, error) {
	...
	if mod.BitLen() == 0 {
		...
		return ioutil.LeftPadBytes([]byte{}, int(modLen)), nil
	}
	return ioutil.LeftPadBytes(base.Exp(base, exp, mod).Bytes(), int(modLen)), nil
}
