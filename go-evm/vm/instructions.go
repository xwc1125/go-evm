...
func opSignExtend(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	if back.Cmp(big.NewInt(31)) < 0 {
		...
		num := stack.pop()
		mask := back.Lsh(params.Big1, bit)
		mask.Sub(mask, params.Big1)
...
func opBlockhash(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	n := interpreter.intPool.get().Sub(interpreter.evm.BlockNumber, params.Big257)
...
func makePush(size uint64, pushByteSize int) executionFunc {
	return func(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
		...
		stack.push(integer.SetBytes(ioutil.RightPadBytes(contract.Code[startMin:endMin], pushByteSize)))
