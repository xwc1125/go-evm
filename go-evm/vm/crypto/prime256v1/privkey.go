...
// ecdsa.PrivateKeyPrivateKey wraps an ecdsa.PrivateKey as a convenience mainly for signing
...
// PrivKeyFromScalar is the same as PrivKeyFromBytes in P256.
...
func GenerateKey(rand io.Reader) (priv []byte, x,
	y *big.Int, err error) {
	key, err := ecdsa.GenerateKey(P256(), rand)
	priv = key.D.Bytes()
	x = key.PublicKey.X
	y = key.PublicKey.Y
