...
var (
	...
	order     = new(big.Int).Set(P256().Params().N)
...
func hashToInt(hash []byte) *big.Int {
	orderBits := P256().Params().N.BitLen()
...
func RecoverCompact(signature, hash []byte) (*PublicKey, bool, error) {
	...
	//iteration := int((signature[len(signature)-1] - 27) & ^byte(4))
	...
	//return key, ((signature[len(signature)-1] - 27) & 4) == 4, nil
...
func bits2octets(in []byte, rolen int) []byte {
	z1 := hashToInt(in)
	z2 := new(big.Int).Sub(z1, P256().Params().N)
	if z2.Sign() < 0 {
...
func VerifySignature(pubkey, msg, signature []byte) bool {
	...
	publicKey, e := ParsePubKey(pubkey)
	if e != nil {
		log.Println("prime256v1", "ParsePubKey", e)
	}
	verify := sig.Verify(msg, publicKey)
	return verify
}
