...
import (
	...
	"github.com/xwc1125/go-evm/pkg/hashmap"
...
var (
	...
	emptyCode = keccak.Keccak256Hash(nil)
)
...
func New(root types.Hash, db Database) (*StateDB, error) {
	//cache, _ := lru.New(codeSizeCacheSize)
	hashMap := hashmap.New()
	err := db.OpenTrie(root)
	if err != nil {
		return nil, err
	}
	return &StateDB{
...
func (self *StateDB) SetCode(addr types.Address, code []byte) {
	...
	if stateObject != nil {
		stateObject.SetCode(keccak.Keccak256Hash(code), code)
	}
}
...
//	var proof proofList
//	err := self.trie.Prove(keccak.Keccak256(a.Bytes()), 0, &proof)
...
//	}
//	err := trie.Prove(keccak.Keccak256(key.Bytes()), 0, &proof)
//	return [][]byte(proof), err
//}
...
func (self *StateDB) deleteStateObject(stateObject *stateObject) {
	...
	addr := stateObject.Address()
	remove, b := self.Lru.Remove(string(addr[:]))
	_ = remove
	//present := self.Lru.Remove(addr[:])
	fmt.Println("present", b)
...
func (self *StateDB) getStateObject(addr types.Address) (stateObject *stateObject) {
	...
	value, _, ok := self.Lru.Get(string(addr[:]))
	if !ok ||value==nil{
		fmt.Println("getStateObject", ok)
